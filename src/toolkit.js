...
export const buildAPIResponse = (statusCode, additionalHeaders, rawBody, version) => {
  ...
  const contentType = statusCode >= 400
    ? 'application/problem+json'
    : `application/vnd.photobox.${version}+json`;
...
export const putItemHandler = async (event, context, callback) => {
  const item = event;
  if (item.path.id) {
    item.body.uuid = event.path.id;
  }

  await client.put({ "TableName": 'toolkitt', Item: item.body }).promise();

  return done(null, event.body, callback);
}

export const deleteHandler = async (event, context, callback) => {
  await client.delete({ "TableName": 'toolkitt', Key: { "firstName": event.pathParameters.id } }).promise();
